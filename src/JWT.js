...
export function createJWT ({address, signer}, payload) {
  ...
  const signingInput = [ENCODED_HEADER, encodeSection({iss: address, iat: Math.floor(Date.now() / 1000), ...payload})].join('.');
  ...
  return new Promise((resolve, reject) => {
    ...
    if (!isMNID(address)) return reject(new Error('No MNID passed'))

    didJWT.createJWT({aud: 'did:uport:${address}', exp: 1957463421, name: 'uPort Developer'},
                     {issuer: 'did:uport:${address}', signer}).then( response => {
                                                                       jwt = response
                                                                     }
                                                                   );
...
export function verifyJWT ({registry, address}, jwt, callbackUrl = null) {
  return new Promise((resolve, reject) => {
    ...
    if (!isMNID(address)) return reject(new Error('No MNID passed'))
    didJWT.verifyJWT(`did:uport:${jwt}`, {audience: address, callbackUrl: callbackUrl}).then(obj => {
