...
import { SimpleSigner, verifyJWT, decodeJWT } from 'did-jwt'
...
describe('configuration', () => {
  ...
  describe('configNetworks', () => {
    it('should accept a valid network setting', () => {
      const networks = {
        '0x94365e3b': { rpcUrl: 'https://private.chain/rpc', registry: '0x3b2631d8e15b145fd2bf99fc5f98346aecdc394c' },
      }
...
describe('disclose()', () => {
  ...
  async function createAndVerify(params = {}) {
    ...
    return verifyJWT(jwt, { audience: did })
  }
...
describe('createTypedDataSignatureRequest()', () => {
  ...
  it('creates a valid JWT for a typed data request', async () => {
    const jwt = await uport.createTypedDataSignatureRequest(typedData, {
      aud: 'did:ethr:deadbeef',
      sub: 'did:ethr:deadbeef',
    })
...
describe('createVerification()', () => {
  ...
  it('has correct payload in JWT for an attestation', async () => {
    return uport
      .createVerification({ sub: 'did:uport:223ab45', claim: { email: 'bingbangbung@email.com' }, exp: 1485321133 + 1 })
      .then(async jwt => {
